...
const cardStyles = {
  [Rarity.COMMON]: { bg: 'bg-gradient-to-b from-slate-400 to-slate-700', border: 'border-slate-300', text: 'text-slate-100', accent: 'bg-slate-800' },
  [Rarity.RARE]: { bg: 'bg-gradient-to-b from-cyan-400 to-blue-700', border: 'border-cyan-200', text: 'text-cyan-50', accent: 'bg-blue-900' },
  [Rarity.EPIC]: { bg: 'bg-gradient-to-b from-purple-400 to-fuchsia-800', border: 'border-purple-200', text: 'text-purple-50', accent: 'bg-purple-900' },
  [Rarity.LEGENDARY]: { bg: 'bg-gradient-to-b from-yellow-200 via-yellow-500 to-yellow-800', border: 'border-yellow-100', text: 'text-yellow-50', accent: 'bg-yellow-950' },
};
...
const CountryFlag = ({ countryCode }: { countryCode?: string }) => {
    if (!countryCode || countryCode === 'xx') return <Globe className="text-gray-500 w-full h-full p-0.5 opacity-50" />;
    return <img src={`/flags/${countryCode.toLowerCase()}.svg`} alt={countryCode} className="w-full h-full object-cover" onError={(e) => { e.currentTarget.style.display = 'none'; e.currentTarget.parentElement?.classList.add('bg-gray-800'); }} />;
};
...
export const Card: React.FC<CardProps> = ({ player, team, compact, onClick, actionLabel }) => {
  ...
  // State: Oyuncu değiştiğinde resim yolunu hesapla
  const [imgSrc, setImgSrc] = useState(getPlayerImageUrl(player));

  // Oyuncu prop'u değişirse (örneğin listede scroll yapınca) resmi güncelle
  useEffect(() => {
    setImgSrc(getPlayerImageUrl(player));
  }, [player]);

  // Resim bulunamazsa (404), varsayılan resme dön
  const handleError = () => {
    if (imgSrc !== DEFAULT_PLAYER_IMAGE) {
        setImgSrc(DEFAULT_PLAYER_IMAGE);
    }
  };
  ...
  if (compact) {
    return (
        <div onClick={onClick} className={`flex items-center gap-3 p-2 rounded-lg border-l-4 cursor-pointer hover:bg-white/5 transition-all bg-dark-900/50 ${style.border.replace('border', 'border-l')}`}>
            <div className="w-12 h-12 rounded-full overflow-hidden border-2 border-white/10 shrink-0 bg-dark-800 relative">
                <PlayerImage />
            </div>
            <div className="flex-1 min-w-0">
                ...
                <div className="flex justify-between text-xs text-gray-400 items-center">
                     ...
                     <div className="w-4 h-3 rounded-[1px] overflow-hidden opacity-80"><CountryFlag countryCode={flagCode} /></div>
                </div>
            </div>
    ...
    <div className="relative group cursor-pointer transition-transform hover:-translate-y-2 hover:scale-105 duration-300" onClick={onClick}>
             ...
             <div className="relative h-full w-full z-10 flex flex-col">
                <div className="flex h-[60%] relative">
                    <div className="w-[28%] flex flex-col items-center pt-5 gap-1 z-20 border-r border-white/5 bg-black/20 backdrop-blur-[1px]">
                        ...
                        <div className="w-6 h-4 shadow-sm border border-white/20 my-1 overflow-hidden rounded-[2px] bg-black/50"><CountryFlag countryCode={flagCode} /></div>
                        {team && <div className="w-7 h-7 mt-1 opacity-90"><TeamLogo team={team} size="w-7 h-7" /></div>}
                    </div>
                    <div className="absolute right-0 top-3 w-[75%] h-full z-10 overflow-hidden">
                        <PlayerImage />
                    </div>
                </div>
                <div className="flex-1 flex flex-col justify-end relative">
                    <div className="text-center mb-1 relative z-20 px-2">
                        <h2 className="text-2xl font-display font-black text-white uppercase tracking-wide leading-none drop-shadow-lg truncate">{player.name}</h2>
      ...
      {actionLabel && (
          <div className="absolute inset-x-0 bottom-6 flex justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-50">
             <button className="px-5 py-2 bg-white text-black font-bold uppercase rounded-full text-xs shadow-[0_0_20px_rgba(255,255,255,0.5)] transform hover:scale-105 transition-transform">{actionLabel}</button>
          </div>
      )}
    </div>
  );
